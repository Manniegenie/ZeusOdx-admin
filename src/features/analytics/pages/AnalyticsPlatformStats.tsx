...
export function AnalyticsPlatformStats() {
  ...
  useEffect(() => {
    titleCtx?.setTitle('Platform Stats');
    titleCtx?.setBreadcrumb(['Analytics', 'Platform Stats']);
  }, [titleCtx]);
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      {(dateFrom || dateTo) && (
        <p className="text-xs text-blue-600 bg-blue-50 px-3 py-1.5 rounded-md inline-block">
          Utility &amp; profits filtered by date — wallet balances show current totals
        </p>
      )}
        ...
        <>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="p-4 bg-blue-50 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-blue-700">{formatCurrency(stats.walletBalances.totalUsd)}</p>
                </div>
                <div className="p-4 bg-green-50 rounded-lg">
                  <p className="text-sm text-gray-600">Total (Naira)</p>
                  <p className="text-2xl font-bold text-green-700">{formatCurrency(stats.walletBalances.totalNaira, 'NGN')}</p>
                </div>
                <div className="p-4 bg-yellow-50 rounded-lg">
                  <p className="text-sm text-gray-600">Pending (USD)</p>
                  <p className="text-2xl font-bold text-yellow-700">{formatCurrency(stats.walletBalances.totalPendingUsd)}</p>
                </div>
                <div className="p-4 bg-purple-50 rounded-lg">
                  <p className="text-sm text-gray-600">Grand Total (USD)</p>
                  <p className="text-2xl font-bold text-purple-700">{formatCurrency(stats.walletBalances.grandTotalUsd)}</p>
                </div>
              </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Airtime, Data, Electricity, Cable TV, and more</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                <div className="p-4 bg-yellow-50 rounded-lg">
                  <p className="text-sm text-gray-600">Total Spent (Naira)</p>
                  <p className="text-2xl font-bold text-yellow-700">{formatCurrency(stats.utilitySpending.totalNaira, 'NGN')}</p>
                </div>
                <div className="p-4 bg-blue-50 rounded-lg">
                  <p className="text-sm text-gray-600">Total Spent (USD)</p>
                  <p className="text-2xl font-bold text-blue-700">{formatCurrency(stats.utilitySpending.totalUsd)}</p>
                </div>
                <div className="p-4 bg-gray-50 rounded-lg">
                  <p className="text-sm text-gray-600">Total Transactions</p>
                  <p className="text-2xl font-bold text-gray-700">{stats.utilitySpending.totalTransactions.toLocaleString()}</p>
                </div>
              </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Fees and markdown profits from withdrawals and swaps</CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <div className="mt-6 p-4 bg-green-50 rounded-lg">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <p className="text-xl font-bold text-green-700">{formatCurrency(stats.profits.summary.totalDirectFeesNaira, 'NGN')}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Direct Fees (USD)</p>
                    <p className="text-xl font-bold text-green-700">{formatCurrency(stats.profits.summary.totalDirectFeesUsd)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Est. Markdown Profit</p>
                    <p className="text-xl font-bold text-green-700">{formatCurrency(stats.profits.summary.totalEstimatedMarkdownProfit, 'NGN')}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Est. Markdown (USD)</p>
                    <p className="text-xl font-bold text-green-700">{formatCurrency(stats.profits.summary.totalEstimatedMarkdownProfitUsd)}</p>
                  </div>
                </div>
              </div>
            </CardContent>
