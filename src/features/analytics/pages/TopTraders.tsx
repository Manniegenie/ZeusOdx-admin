              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-right py-2 px-4">NGNZ Volume</th>
                    <th className="text-right py-2 px-4">Est. USD Volume</th>
                    <th className="text-left py-2 px-4">Tokens Traded</th>
                    <th className="text-right py-2 px-4">Last Trade</th>
                  </tr>
                </thead>
                <tbody>
                  {traders.map((trader, index) => (
                    <tr key={trader.userId} className={`border-b hover:bg-gray-50 ${index < 3 ? 'bg-yellow-50/50' : ''}`}>
                      <td className="py-2.5 px-4 font-medium text-lg">{rankBadge(index)}</td>
                      ...
                      <td className="text-right py-2.5 px-4 font-bold">{trader.tradeCount.toLocaleString()}</td>
                      <td className="text-right py-2.5 px-4">{formatCurrency(trader.totalNgnzVolume)}</td>
                      <td className="text-right py-2.5 px-4">{formatUsd(trader.estimatedUsdVolume)}</td>
                      <td className="py-2.5 px-4">
                        <div className="flex flex-wrap gap-1">
                          {trader.currencies.slice(0, 5).map((c) => (
                            <span key={c} className="text-xs bg-gray-100 px-1.5 py-0.5 rounded">{c}</span>
                          ))}
                          {trader.currencies.length > 5 && (
                            <span className="text-xs text-gray-400">+{trader.currencies.length - 5}</span>
                          )}
                        </div>
                      </td>
                      <td className="text-right py-2.5 px-4 text-gray-500 text-xs">
                        {new Date(trader.lastTradeAt).toLocaleDateString()}
                      </td>
                    </tr>
                  ))}
                </tbody>
